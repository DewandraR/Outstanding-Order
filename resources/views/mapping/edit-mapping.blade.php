...
<div class="container" style="max-width: 720px;">
    ...
    <form method="post" action="{{ route('mapping.update',$mapping->Id) }}">
        @csrf
        @method('PUT')

        <div class="mb-3">
            <label class="form-label">IV_WERKS</label>
            <input type="text" name="IV_WERKS" value="{{ old('IV_WERKS',$mapping->IV_WERKS) }}" class="form-control" required>
            @error('IV_WERKS') <div class="text-danger small">{{ $message }}</div> @enderror
        </div>

        <div class="mb-3">
            <label class="form-label">IV_AUART</label>
            <input type="text" name="IV_AUART" value="{{ old('IV_AUART',$mapping->IV_AUART) }}" class="form-control" required>
            @error('IV_AUART') <div class="text-danger small">{{ $message }}</div> @enderror
        </div>

        <div class="mb-3">
            <label class="form-label">Deskription</label>
            <input type="text" name="Deskription" value="{{ old('Deskription',$mapping->Deskription) }}" class="form-control" required>
