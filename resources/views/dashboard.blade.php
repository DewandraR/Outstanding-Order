...
<div class="row g-4 mb-4">
    ...
    <div class="col-sm-6 col-xl-3">
        ...
        <div id="toggle-bottlenecks-card" class="card yz-kpi-card card-highlight-warning h-100 shadow-sm" style="cursor: pointer;" title="Klik untuk melihat Potential Bottlenecks">
            <div class="card-body d-flex align-items-center">
                ...
                <div class="ms-3">
                    ...
                    <h4 class="mb-0 fw-bolder"><span id="kpi-so-bottleneck">0</span> <small id="kpi-so-bottleneck-unit">Items</small></h4>
                </div>
            </div>
        </div>
    </div>
          ...
          callbacks: {
            ...
            label: (context) => {
              ...
              if (canvasId === 'chartTopOverdueCustomers' && dataPoint) {
                const total = dataPoint.overdue_count, smg = dataPoint.smg_count, sby = dataPoint.sby_count;
                let details = [];
...
(() => {
        ...
        try {
          ...
          wrap.querySelectorAll('.js-t2row').forEach(row2 => {
            row2.addEventListener('click', async (ev) => {
              ...
              if (!open) { tbody2.classList.add('so-focus-mode'); row2.classList.add('is-focused'); }
              else { tbody2.classList.remove('so-focus-mode'); row2.classList.remove('is-focused'); }

              if (open) { tgt.style.display = 'none'; caret?.classList.remove('rot'); return; }
              tgt.style.display = ''; caret?.classList.add('rot');
        ...
        if (customerRow) {
          ...
          const interval = setInterval(() => {
            ...
            if (soRow) {
              ...
              setTimeout(() => soRow.scrollIntoView({ behavior: 'smooth', block: 'center' }), 500);
            }
            attempts++;
  ...
  if (mappingData) {
    for (const werks in mappingData) {
      mappingData[werks].forEach(item => { auartMap[item.IV_AUART] = item.Deskription; });
    }
  }
  ...
  if (currentView === 'so') {
    ...
    function renderBottlenecksTable(rows) {
      const mappingData = JSON.parse(document.getElementById('dashboard-data-holder').dataset.mappingData || '{}');
      const auartMap2 = {};
      for (const w in mappingData) (mappingData[w]||[]).forEach(m => auartMap2[m.IV_AUART] = m.Deskription);
      ...
      document.getElementById('close-bottlenecks')?.addEventListener('click', () => bottleneckBox.style.display = 'none');
    }
    ...
    if (ctxLocationStatus) {
      ...
      if (locationData.length === 0) {
        ...
        const onTime = labels.map(loc => (locationData.find(d => d.location === loc)?.on_time_value || 0));
        const overdue = labels.map(loc => (locationData.find(d => d.location === loc)?.overdue_value || 0));
        new Chart(ctxLocationStatus, {
          type: 'bar',
          data: { labels, datasets: [
            { label: 'On Time', data: onTime,  backgroundColor: 'rgba(75, 192, 192, 0.7)' },
            { label: 'Overdue', data: overdue, backgroundColor: 'rgba(255, 99, 132, 0.7)' }
          ]},
          options: {
            scales: { x: { stacked: true }, y: { stacked: true, beginAtZero: true } },
            plugins: { tooltip: { callbacks: { label: (ctx) => `${ctx.dataset.label}: ${formatFullCurrency(Number(ctx.raw) || 0, 'USD')}` } } }
          }
        });
      }
    }
      ...
      if (total === 0) {
        ...
        const soUrgencyChart = new Chart(ctxSoUrgency, {
          ...
          data: {
            ...
            datasets: [{ data: [agingData.overdue_over_30, agingData.overdue_1_30, agingData.due_this_week, agingData.on_time],
          ...
          options: {
            ...
            onClick: async (evt, elements) => {
              ...
              const map = { 'Overdue > 30 Days': 'overdue_over_30','Overdue 1-30 Days':'overdue_1_30','Due This Week':'due_this_week','On Time':'on_time' };
...
function buildTable(rows) {
  ...
  const currencyToDisplay = dataHolder.dataset.selectedType === 'lokal' ? 'IDR' : 'USD';
  ...
  if (dataHolder.dataset.selectedType === 'lokal') {
  ...
  const poTypeToCodes = {
    'KMI Export SBY':  { werks: '2000', auart: 'ZOR1' },
    'KMI Local SBY':   { werks: '2000', auart: 'ZOR3' },
    'KMI Replace SBY': { werks: '2000', auart: 'ZRP1' },
    'KMI Export SMG':  { werks: '3000', auart: 'ZOR2' },
    'KMI Local SMG':   { werks: '3000', auart: 'ZOR4' },
    'KMI Replace SMG': { werks: '3000', auart: 'ZRP2' },
  };
  ...
  const getCodesFromItem = (item) => {
    if (item.IV_WERKS_PARAM && item.IV_AUART_PARAM) return { werks: String(item.IV_WERKS_PARAM), auart: String(item.IV_AUART_PARAM) };
    if (item.IV_WERKS && item.IV_AUART)             return { werks: String(item.IV_WERKS),       auart: String(item.IV_AUART) };
    if (item.WERKS && item.AUART)                   return { werks: String(item.WERKS),          auart: String(item.AUART) };
    const key = (item.Deskription || '').trim();
    return poTypeToCodes[key] || { werks: '', auart: '' };
  };
  ...
  if (performanceTbody) {
    if (!performanceData || performanceData.length === 0) {
      ...
      performanceData.forEach(item => {
        ...
        const { werks, auart } = getCodesFromItem(item);
        const totalOverdueForBar = overdueSo;
        const pct = (n) => totalOverdueForBar > 0 ? (n / totalOverdueForBar * 100).toFixed(2) : 0;
        ...
        barChartHtml += seg(item.overdue_1_30,   pct(item.overdue_1_30),   '#ffc107', '1_30',  '1–30 Days');
        barChartHtml += seg(item.overdue_31_60,  pct(item.overdue_31_60),  '#fd7e14', '31_60', '31–60 Days');
        barChartHtml += seg(item.overdue_61_90,  pct(item.overdue_61_90),  '#dc3545', '61_90', '61–90 Days');
        barChartHtml += seg(item.overdue_over_90,pct(item.overdue_over_90),'#8b0000','gt_90',  '>90 Days');
        barChartHtml += '</div>';
    ...
    performanceTbody.addEventListener('click', async (e) => {
      ...
      const rowTitle = seg.closest('tr')?.querySelector('td:first-child .fw-bold')?.textContent?.trim() || 'Selected';
      ...
      if (!overlay) {
        ...
        overlay.style.cssText = 'position:absolute;inset:0;background:var(--bs-card-bg,#fff);z-index:10;display:flex;padding:1rem;';
      ...
      const showError = (msg) => {
        ...
        document.getElementById('close-po-perf-details')?.addEventListener('click', () => overlay.remove());
      };
      ...
      try {
        if (!werks || !auart) throw new Error('Parameter plant (werks) atau order type (auart) kosong.');
  ...
  function renderPoOverdueTable(rows, labelText) {
    ...
    document.getElementById('closePoOverdueOverlay')?.addEventListener('click', () => {
      container.style.display = 'none'; container.innerHTML = ''; container.removeAttribute('style');
    });
  }
  ...
  if (ctxSmallQty) {
    if (smallQtyDataRaw.length === 0) {
      ...
      smallQtyDataRaw.forEach(item => {
        if (!customerMap.has(item.NAME1)) customerMap.set(item.NAME1, { '3000': 0, '2000': 0 });
      ...
      new Chart(ctxSmallQty, {
        ...
        options: {
          indexAxis: 'y',
          scales: { x: { beginAtZero: true, title: { display: true, text: 'Item (With Qty Outstanding ≤ 5)' } } },
  ...
  async function loadSoUrgencyDetails(statusKey, labelText) {
    ...
    if (!container) return;
    Object.assign(container.style, { position: 'absolute', top: '0', left: '0', width: '100%', height: '100%', background: 'var(--bs-card-bg, white)', zIndex: '10', display: 'flex', padding: '1rem' });
    container.innerHTML = `
      <div class="card yz-chart-card shadow-sm h-100 w-100">
        <div class="card-body d-flex flex-column">
          <div class="d-flex justify-content-between align-items-center">
  ...
  function renderSoUrgencyTable(rows, labelText) {
    ...
    document.getElementById('closeSoUrgencyDetails')?.addEventListener('click', () => {
      container.removeAttribute('style'); container.innerHTML = '';
    });
  }
})();
</script>
