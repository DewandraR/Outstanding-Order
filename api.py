...
Mode CLI (tanpa HTTP):
  ...
  python api.py --sync_comp --werks 2000 --auart ZOR3
...
# Abaikan CTRL+C di level signal (biar thread pool nggak putus di tengah bulk insert)
try:
...
def connect_sap_with_fallback(username: str, password: str) -> Tuple[Connection, str]:
    """
    Coba login pakai user yg diminta.
    Kalau user itu 'auto_email' dan gagal, fallback ke 'sap_automation'.
    """
    primary_user = (username or "auto_email").strip()
    fallback_user = os.getenv("SAP_FALLBACK_USER", "sap_automation").strip()

    try:
        print(f"[INFO] Connecting SAP as '{primary_user}'...")
        conn = connect_sap(primary_user, password)
        print(f"[INFO] SAP connected as '{primary_user}'")
        return conn, primary_user
    except Exception as e1:
        if primary_user.lower() == "auto_email":
            print(f"[WARN] Login '{primary_user}' failed: {e1} -> try '{fallback_user}'")
            try:
                conn = connect_sap(fallback_user, password)
                print(f"[INFO] SAP connected as fallback '{fallback_user}'")
                return conn, fallback_user
            except Exception as e2:
                print(f"[ERROR] Fallback '{fallback_user}' failed: {e2}")
                raise
        else:
            print(f"[ERROR] SAP connection failed for '{primary_user}': {e1}")
            raise
...
def get_sap_credentials_from_headers() -> Tuple[str, str]:
    ...
    print(
        f"[INFO] Using SAP user={u} client={DEFAULT_SAP['client']} "
        f"ashost={DEFAULT_SAP['ashost']} sysnr={DEFAULT_SAP['sysnr']} lang={DEFAULT_SAP['lang']}",
        flush=True,
    )
    return (u, p)
...
def fnum(x):
    """Konversi ke float aman (NaN/Inf jadi None)."""
    try:
        if x in ("", None):
            return None
...
def _sap_call_no_decimal_trap(conn: Connection, fm: str, **params):
    """Matikan trap Decimal InvalidOperation di thread ini lalu call RFC."""
    ctx = decimal.getcontext()
    ctx.traps[decimal.InvalidOperation] = False
    return conn.call(fm, **params)

def call_rfc_with_timeout(conn: Connection, seconds: int, fm: str, **params) -> Dict[str, Any]:
    # pastikan CHAR sure di-string-kan
    for key in ("IV_WERKS", "IV_AUART", "IV_BALANCE", "IV_SIAD", "IV_SIPD", "IV_SIPP", "IV_SIPM"):
...
def fetch_pairs_from_maping(filter_werks=None, filter_auart=None, limit: int = None):
    """Ambil pasangan IV_WERKS / IV_AUART dari tabel maping."""
    db = connect_mysql()
...
COMMON_SO_COLS = [
    ...
    # kolom tambahan (sebelum fetched_at)
    "ASSY", "PAINTM", "PACKGM",
    "PRSM", "PRSM2",
...
def ensure_tables():
    ...
    try:
        ...
        # T2 + perbaikan tipe
        cur.execute("CREATE TABLE IF NOT EXISTS so_yppr079_t2 LIKE so_yppr079_t1;")
        cur.execute("""
        ...
        # Tambahan minor KALAB2 kalau belum ada
        ...
        # ---------- TABEL *_comp ----------
        try:
            cur.execute("CREATE TABLE IF NOT EXISTS so_yppr079_t1_comp LIKE so_yppr079_t1;")
            cur.execute("CREATE TABLE IF NOT EXISTS so_yppr079_t2_comp LIKE so_yppr079_t2;")
            cur.execute("CREATE TABLE IF NOT EXISTS so_yppr079_t3_comp LIKE so_yppr079_t3;")
            cur.execute("CREATE TABLE IF NOT EXISTS so_yppr079_t4_comp LIKE so_yppr079_t4;")
        except Exception:
            pass
        ...
        for _tbl in ("so_yppr079_t1_comp", "so_yppr079_t2_comp", "so_yppr079_t3_comp", "so_yppr079_t4_comp"):
            try:
                cur.execute(
                    f"ALTER TABLE {_tbl} CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci"
                )
            except Exception:
                pass
        ...
        try:
            cur.execute("""
                ALTER TABLE so_yppr079_t2_comp
                ADD UNIQUE KEY uq_t2 (IV_WERKS_PARAM, IV_AUART_PARAM, VBELN, POSNR)
            """)
        except Exception:
            pass
        ...
        try:
            cur.execute("""
                ALTER TABLE so_yppr079_t3_comp
                ADD UNIQUE KEY uq_t3 (IV_WERKS_PARAM, IV_AUART_PARAM, VBELN, POSNR)
            """)
        except Exception:
            pass
...
def _bulk_insert_so_plain(cur, table: str, params: list, batch_size: int = 1000):
    """Dipakai T1 (tidak pakai ON DUPLICATE)"""
    if not params: return 0
...
def _bulk_insert_t4_plain(cur, table: str, params: list, batch_size: int = 1000):
    if not params:
        return 0
    sql = f"INSERT INTO {table} ({T4_COL_LIST}) VALUES ({T4_PLACEHOLDERS})"
    total = 0
    for i in range(0, len(params), batch_size):
        chunk = params[i:i+batch_size]
        cur.executemany(sql, chunk)
...
def do_sync(
    ...
    """
    Sinkron Z_FM_YPPR079_SO:

    - iv_balance = ""  -> data normal
      table_suffix = "" -> tabel utama:  so_yppr079_t1..t4

    - iv_balance = "X" -> data complete
      table_suffix = "_comp" -> tabel: so_yppr079_t1_comp..t4_comp

    Mekanisme:
      * Jika full mapping (tanpa filter & limit) dan semua pair sukses:
            TRUNCATE semua tabel (sesuai suffix) lalu isi ulang full.
      * Selain itu: per-pair (hapus WHERE WERKS/AUART lalu insert lagi).
    """
    ensure_tables()

    table_suffix = table_suffix or ""
    tbl1 = "so_yppr079_t1" + table_suffix
    tbl2 = "so_yppr079_t2" + table_suffix
    tbl3 = "so_yppr079_t3" + table_suffix
    tbl4 = "so_yppr079_t4" + table_suffix
    ...
    print(f"[INFO] Start do_sync SO (IV_BALANCE='{iv_balance or ''}', suffix='{table_suffix}') @ {start_ts}", flush=True)

    pairs = fetch_pairs_from_maping(filter_werks, filter_auart, limit)
    total = len(pairs)
    full_mapping_requested = (filter_werks is None and filter_auart is None and (limit is None))
    if not pairs:
    ...
    t1_buf_map: Dict[Tuple[str,str], List[tuple]] = {}
    t2_buf_map: Dict[Tuple[str,str], List[tuple]] = {}
    t3_buf_map: Dict[Tuple[str,str], List[tuple]] = {}
    t4_buf_map: Dict[Tuple[str,str], List[tuple]] = {}
    ...
    try:
        for idx, p in enumerate(pairs, 1):
            ...
            key = (werks, auart)
            try:
                print(f"[{idx}/{total}] Call RFC {RFC_NAME_SO} WERKS={werks} AUART={auart} IV_BALANCE='{iv_balance or ''}'", flush=True)
                ...
                t1_rows = [_so_row_params(werks, auart, row, now) for row in t1]
                t2_rows = [_so_row_params(werks, auart, row, now) for row in t2]
                t3_rows = [_so_row_params(werks, auart, row, now) for row in t3]
                t4_rows = [_so_t4_row_params(werks, auart, row, now) for row in t4]

                t1_buf_map[key] = t1_rows
                t2_buf_map[key] = t2_rows
                t3_buf_map[key] = t3_rows
                t4_buf_map[key] = t4_rows

                total_t1 += len(t1_rows)
                total_t2 += len(t2_rows)
                total_t3 += len(t3_rows)
                total_t4 += len(t4_rows)

                success_pairs.append(key)
                summary.append({
                    "werks": werks,
                    "auart": auart,
                    "t1_saved": len(t1_rows),
                    "t2_saved": len(t2_rows),
                    "t3_saved": len(t3_rows),
                    "t4_saved": len(t4_rows),
                    "iv_balance": iv_balance or "",
                    "table_suffix": table_suffix or "",
                    "ok": True,
                })
                ...
                summary.append({
                    ...
                    "error": f"timeout: {te}",
                    "iv_balance": iv_balance or "", "table_suffix": table_suffix or "",
                    "ok": False,
                })
            except Exception as e:
    ...
    try:
        if do_full_truncate:
            print(f"[INFO] FULL mapping & semua OK -> TRUNCATE {tbl1}, {tbl2}, {tbl3}, {tbl4}", flush=True)
            cur.execute("SET FOREIGN_KEY_CHECKS=0")
            for tbl in (tbl1, tbl2, tbl3, tbl4):
                cur.execute(f"TRUNCATE TABLE {tbl}")
            cur.execute("SET FOREIGN_KEY_CHECKS=1")

            for (werks, auart) in success_pairs:
                key = (werks, auart)
                t1_rows = t1_buf_map.get(key, [])
                t2_rows = t2_buf_map.get(key, [])
                t3_rows = t3_buf_map.get(key, [])
                t4_rows = t4_buf_map.get(key, [])

                inserted_t1 += _bulk_insert_so_plain(cur, tbl1, t1_rows)
                inserted_t2 += _bulk_insert_so_upsert(cur, tbl2, t2_rows)
                inserted_t3 += _bulk_insert_so_upsert(cur, tbl3, t3_rows)
                inserted_t4 += _bulk_insert_t4_plain(cur, tbl4, t4_rows)
        else:
            print(f"[INFO] PARTIAL REFRESH (tanpa TRUNCATE) untuk {len(success_pairs)} pair sukses", flush=True)
            for (werks, auart) in success_pairs:
                key = (werks, auart)
                args = (werks, auart)

                # delete lama per-pair
                for tbl in (tbl1, tbl2, tbl3, tbl4):
                    cur.execute(
                        f"DELETE FROM {tbl} WHERE IV_WERKS_PARAM=%s AND IV_AUART_PARAM=%s",
                        args,
                    )
    ...
    print(
        f"[INFO] Done SO sync (IV_BALANCE='{iv_balance or ''}', suffix='{table_suffix}') "
        f"totals SAP: t1={total_t1} t2={total_t2} t3={total_t3} t4={total_t4}  "
        f"inserted: t1={inserted_t1} t2={inserted_t2} t3={inserted_t3} t4={inserted_t4}",
        flush=True,
    )
    ...
    return {
        ...
        "all_pairs_ok": all_ok,
        "success_pairs": [{"werks": w, "auart": a} for (w, a) in success_pairs],
        "full_mapping_requested": full_mapping_requested,
        "do_full_truncate": do_full_truncate,
        "iv_balance": iv_balance or "",
        "table_suffix": table_suffix or "",
    }
...
def do_sync_stock(timeout_sec: int = 3000):
    """
    Jalankan Z_FM_YSDR048 4x:
      IV_SIAD = X  -> stock_assy
      IV_SIPD = X  -> stock_ptg
      IV_SIPP = X  -> stock_pkg
      IV_SIPM = X  -> stock_ptg_m
    Semua tabel stok di-TRUNCATE dulu, lalu diisi ulang.
    """
    ensure_stock_tables()
    ...
    start_ts = datetime.datetime.now()
    print(f"[INFO] Start do_sync_stock @ {start_ts}", flush=True)
    ...
    try:
        for param_name, param_value in sync_params.items():
            ...
            try:
                print(f"[INFO] Call RFC {RFC_NAME_STOCK} {param_name}='{param_value}' -> {table_name}", flush=True)
                ...
                print(f"[ERROR] {param_name} timeout: {te}", flush=True)
                summary.append({"param": param_name, "error": f"timeout: {te}"})
            except Exception as e:
                print(f"[ERROR] {param_name} error: {e}", flush=True)
    ...
    inserted_counts = {}
    try:
        print("[INFO] TRUNCATE semua tabel stok...", flush=True)
        cur.execute("SET FOREIGN_KEY_CHECKS=0")
        for table_name in buffers.keys():
            cur.execute(f"TRUNCATE TABLE {table_name}")
        cur.execute("SET FOREIGN_KEY_CHECKS=1")

        print("[INFO] Insert data stok buffered...", flush=True)
        ...
        db.rollback()
        print(f"[ERROR] DB insert failed (stock): {e}", flush=True)
        return {"ok": False, "error": f"DB insert failed: {e}", "summary": summary}
    finally:
        cur.close(); db.close()

    done_ts = datetime.datetime.now()
    print(f"[INFO] Done Stock sync @ {done_ts} inserted={inserted_counts}", flush=True)
...
def home():
    return (
        "OK. POST /api/yppr079/sync (SO normal) atau /api/ysdr048/sync (Stock).",
        200,
...
def sync_yppr079_http():
    """
    Endpoint HTTP untuk sinkronisasi SO NORMAL (tanpa IV_BALANCE, ke tabel utama).
    Untuk COMPLETE gunakan CLI --sync_comp (sengaja dipisah).
    """
    ensure_tables()
    ...
    timeout_sec  = request.args.get("timeout", default=90000, type=int)

    os.environ["SAP_USERNAME"] = u
    os.environ["SAP_PASSWORD"] = p

    result = do_sync(
        filter_werks=filter_werks,
        filter_auart=filter_auart,
        limit=limit_param,
...
if __name__ == "__main__":
    ...
    # PERILAKU BARU:
    # --sync      -> HANYA SO NORMAL (tanpa IV_BALANCE) ke so_yppr079_t1..t4
    # --sync_comp -> HANYA SO COMPLETE (IV_BALANCE='X') ke so_yppr079_t1..t4_comp
    parser.add_argument("--sync", action="store_true",
                        help="Jalankan sinkronisasi SO NORMAL (tanpa IV_BALANCE, ke so_yppr079_t1..t4)")
    parser.add_argument("--sync_comp", action="store_true",
                        help="Jalankan sinkronisasi SO COMPLETE (IV_BALANCE='X') ke tabel *_comp saja")

    parser.add_argument("--werks", type=str, help="Filter IV_WERKS (untuk --sync / --sync_comp)")
    parser.add_argument("--auart", nargs="+", help="Filter IV_AUART (list, misal: ZOR1 ZOR3)")
    parser.add_argument("--limit", type=int, help="Batas jumlah pair dari tabel maping")

    parser.add_argument("--sync_stock", action="store_true", help="Jalankan sinkronisasi Stock Z_FM_YSDR048")
    parser.add_argument("--timeout", type=int, default=90000, help="Timeout per panggilan RFC (detik)")
    ...
    if args.sync:
        ...
        res = do_sync(
            ...
            iv_balance="",       # <--- normal
            table_suffix="",     # <--- tabel utama
        )
        print(json.dumps(to_jsonable(res), ensure_ascii=False, indent=2))

    elif args.sync_comp:
        # HANYA data complete ke *_comp
        res = do_sync(
            filter_werks=args.werks,
            filter_auart=args.auart,
            limit=args.limit,
            timeout_sec=args.timeout,
            iv_balance="X",      # <--- complete
            table_suffix="_comp" # <--- tabel _comp
        )
        print(json.dumps(to_jsonable(res), ensure_ascii=False, indent=2))

    elif args.sync_stock:
        res = do_sync_stock(timeout_sec=args.timeout)
        print(json.dumps(to_jsonable(res), ensure_ascii=False, indent=2))
